...
(function ($) {
	...
	function fileManagerObj(_handler) {
		...
		this.load = function (callback) {
			...
			self.authenticate(function (response) {
				...
				if (self.mkdir.enabled) {
					...
					uiHtml += '<div class="wysiwyg-files-action-mkFile" title="Создать файл	"></div>';
				}
					...
					var fileManagerUI = new $.wysiwyg.dialog(_handler, {
						...
						"open": function (e, dialog) {
							...
							dialog.find("input[name=submit]").bind("click", function () {
								var file = cfg_fm.dir_up+dialog.find("input[name=url]").val();
								fileManagerUI.close();
								self.loaded = false;
								callback(file);
							});
							...
							// Create File
							$(".wysiwyg-files-action-mkFile").bind("click", function (e) {
							 e.preventDefault();	
			    				var uiHtml = '<div>' +
								...
								var mkdirDialog = new $.wysiwyg.dialog(null, {
									...
									"open": function (e, _dialog) {
										_dialog.find("input[name=create]").bind("click", function () {
											self.mkFile(_dialog.find("input[name=newName]").val(), function (response) {
												self.loadDir();
												mkdirDialog.close();
											});
										});
		...
		this.authenticate = function (callback) {			
			...
			//alert(cfg_fm.passw);
			$.getJSON(self.handler, {"action": "auth", "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up}, 
		...
		this.loadDir = function () {
			...
			$.getJSON(self.handler, {"dir": self.curDir, "action": "list", "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up}, function (json) {
 ...
 */
		...
		this.removeFile = function (type, callback) {
			...
			var self = this;
			$.getJSON(self.remove.handler, { "action": "remove", "type": type, "dir": self.curDir, "file": self.selectedFile, "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up }, function (json) {
				if (json.success) {
					alert(json.data);
				} else {
					alert(json.error);
		...
		this.renameFile = function (type, newName, callback) {
			...
			var self = this;
			$.getJSON(self.rename.handler, { "action": "rename", "type": type, "dir": self.curDir, "file": self.selectedFile, "newName": newName , "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up }, function (json) {
				if (json.success) {
					alert(json.data);
				} else {
					alert(json.error);
		...
		this.mkDir = function (newName, callback) {
			...
			var self = this;
			$.getJSON(self.mkdir.handler, { "action": "mkdir", "dir": self.curDir, "newName": newName , "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up }, function (json) {
				if (json.success) {
					alert(json.data);
				} else {
					alert(json.error);
		...
		// Make File Method
		this.mkFile = function (newName, callback) {
			if (!this.loaded) { return false; }
			if (!this.mkdir.enabled) { console.log("$.wysiwyg.fileManager: handler: mkdir is disabled."); return false; }
			var self = this;
			$.getJSON(self.mkdir.handler, { "action": "mkFile", "dir": self.curDir, "newName": newName , "auth": cfg_fm.passw,"dir_up":cfg_fm.dir_up }, function (json) {
				if (json.success) {
					alert(json.data);
				} else {
					alert(json.error);
		...
		this.loadUploadUI = function () {
			...
			var uiHtml = '<form enctype="multipart/form-data" method="post" action="' + self.upload.handler + '">' +
							...
							'<input type="text" name="dir_up" style="display:none;" value="' + cfg_fm.dir_up + '" />' +
