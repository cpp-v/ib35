...
//include_once($_SERVER['DOCUMENT_ROOT']."/assets/cppv/aset_test.php"); //delete_test_file();
                     ...
                     // add_test_str('$_REQUEST='.print_r($_REQUST,true));
...
header_out();       
	...
	if($subStr=='SELE' || $subStr=='SHOW'){			
echo "<tbody>";
		$sth=qwery($sqlText);                    
		$n_f=$sth->columnCount(); $pp=0;
		while($row=$sth->fetch())
		{
	    	echo "<tr>";   echo "<td>",++$pp,"</td>";//порядковый номер
...
function GFG_field_list_load()
{
  ...
  //$sth = qwery("select * from ".$_REQUEST["table_name"]." limit 1");
    $subStr=substr($_REQUEST["sql_text"], 0, 4);
    $subStr=strtoupper($subStr); 
  
  $qrSQL=$_REQUEST["sql_text"];  
  $posLimit=strpos($qrSQL, "limit");
  
  if($subStr=='SHOW') $posLimit=100000;//хак, чтобы не подставлял строку лимита для SHOW
  
  if(!$posLimit) $sth =qwery($qrSQL." limit 1");
  else  $sth =qwery($qrSQL);
  
  $n_f=$sth->columnCount();
  $CFG["field_count"]=$n_f+1;
  $CFG["fields_arr"]=array();
  
        $CFG["fields_arr"][0]=array();
        $CFG["fields_arr"][0]["name"]="N";     
        $CFG["fields_arr"][0]["len"]='';
        $CFG["fields_arr"][0]["type"]='';
  for($i=0;$i<$CFG["field_count"];$i++)
  {
  	     $colMeta=$sth->getColumnMeta($i); 
        $CFG["fields_arr"][$i+1]=array();
        $CFG["fields_arr"][$i+1]["name"]=$colMeta['name'];     
        $CFG["fields_arr"][$i+1]["len"]=$colMeta['len'];
        $CFG["fields_arr"][$i+1]["type"]=$colMeta['native_type'];
  }
  return;
